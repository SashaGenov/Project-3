...
func (w Walking) Calories() float64 {
	...
	mrost := w.Height / CmInM
	if mrost == 0 {
		return 0
	}
	stepen := math.Pow((w.meanSpeed()*KmHInMsec), 2) / mrost
	umn1 := CaloriesWeightMultiplier * w.Weight
	umn2 := CaloriesSpeedHeightMultiplier * w.Weight
	return (umn1 + stepen*umn2) * w.Duration.Minutes()
}
...
func (s Swimming) meanSpeed() float64 {
	...
	sa := float64(s.LengthPool * s.CountPool)
	st := float64(MInKm) / s.Duration.Hours()
	return sa / st
}
